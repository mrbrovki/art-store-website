...
const Products: FC<{ category: CategoryName; pexelsKey: string }> = ({
  category: name,
  pexelsKey,
}) => {
  const {
  ...
  const fetchData = async () => {
    ...
    const res = await fetch(
      `https://api.pexels.com/v1/search/?page=${pages[name]}&per_page=${per_page}&query=${query}`,
      {
        method: "GET",
        headers: {
    ...
    dispatch({
      type: "PHOTOS",
      payload: { ...photos, [name]: [...photos[name], ...data.photos] },
    });
  };
